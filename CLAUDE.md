...
- **Scanners:** `us/scan_us_all.py` — single file, eight: breadth (`scanner_us_breadth`), fresh breakouts 3M/6M/1Y (`scanner_us_breakouts`), rvol (`scanner_us_rvol`), EP 6-mo leaderboard gap≥1%/move≥7%/vol≥3× with `earnings_date` stamped from the NASDAQ calendar (`scanner_us_ep`), VCP ret3m≥25%+15d range<15% (`scanner_us_vcp`), **groups** = sector + ~29 granular-theme (semis/software/biotech/banks/…) D/W/M performance with tracked-ETF returns + top-3 leaders (`scanner_us_groups`; `SECTOR_ETFS` + `THEMES` maps NASDAQ industry tags → themes; replaced `scanner_us_sectors`), **earnings** = last ~7 sessions + next 7 days from NASDAQ calendar, BEAT/MISS/INLINE vs consensus via the earnings-surprise endpoint + day `reaction_pct` (`us_earnings`), **news** = Yahoo headlines for today's top movers (|chg|≥3% or rvol≥2, top 25) + SPY/QQQ, deduped on article id, 7-day retention (`us_news`).
