...
| `scanner_vcp_bear` | `(run_date, symbol)` | **Inverse VCP shorts (added 2026-08-12)** — all actives: 3M fall ≤ −15% with top-3 down-day vol ≥1.5× 20d avg, 15d range < 15%, 20DMA falling (vs 10 sessions ago) with close below it; `touching` = 15d high within 2% of the falling 20DMA. Full refresh by `scan_vcp_bear.py` (in `scan_all.py` as `vcp_bear`). Feeds Market Manthan Mandi tab. |
| `scanner_snapback` | `(run_date, symbol)` | **Snapback / undercut & rally (added 2026-08-04)** — F&O only. Intraday low undercuts prior 1M/21 / 3M/63 / 6M/126 / 1Y/252-session min low within the last 5 sessions, then snaps back: `SAME_DAY` (close in top 35% of range) or `D1`/`D2` (close back above `broken_level`). Cols: `low_date`, `extreme_low`, `broken_level`, `snap_type/date/close`, `last_close`, `pct_off_low`, `close_pos`, `flush/snap_vol_ratio` (vs 20d avg), `score` 0-4 (+1 window≥3M, +1 same-day bar, +1 flush vol≥1.5×, +1 snap vol≥1.5×). Full refresh by `scan_snapback.py` (in `scan_all.py` as `snapback`). Dashboard **🪝 Snapback** tab + `kpi-snap` chip + `SNP` scan-hit pill. |
...
| `ohlcv_5min` | `(symbol, ts)` | **5-minute bars (added 2026-08-30)** — Yahoo 5m candles, top 1000 by market cap, ~30 trading sessions (Yahoo caps 5m at ~60 days). 75 bars/session (09:15 … 15:25). Loaded by `backfill_intraday.py`; not yet on a schedule — re-run to extend. |
| `metals_ohlcv` | `(symbol, interval, ts)` | **Gold / silver desk feed (added 2026-09-21)** — Yahoo bars for `GC=F`, `SI=F`, `DX-Y.NYB`, `^TNX` at `1d` (730d), `1h` (60d, pruned at 90), `5m` (5d, pruned at 10). Written by `fetch_metals.py` via `metals_ingest.yml` twice hourly Sun–Fri (COMEX trades ~23h). Read by the **Bullion Desk** artifact (claude.ai) through the Supabase connector — no repo dashboard reads it. |
...
| `scanner_trendday` | `symbol` | **Trend Day (added 2026-08-30)** — intraday scanner for stocks building a +6–10% day, rebuilt on every hourly run by `scan_trendday.py` (called from `fetch_hourly.py`). Current session only. Cols: `slot_no` (1=09:15 bar … 7=15:15), `h1_ret` (10:15 close vs prev close), `h1_rvol` (first-hour volume ÷ that stock's own 10-day avg first-hour volume), `h1_pos`, `h1_range`, `h2_bar` (10:15→11:15 return), `v2_v1`, `open_vs_low`, `held_open`, `gap_pct`, `cum_pct`, `score` = modelled P(close ≥ +6%) from the 10:15/11:15 bars only, `stage` WATCH/SETUP/CONFIRMED/FADING, `triggered_at`/`trigger_stage`/`trigger_slot` (stamped the first time the row reaches SETUP/CONFIRMED and never overwritten). Dashboard **🔥 Trend Day** tab + `kpi-trend` chip. |
| `sector_history` | `(name, ts)` | **Intraday sector series (added 2026-07-20)** — 1-min bars appended by `kite_ticks.py` while streaming: per-`stocks.sector` average `chg_pct` of F&O ticks (`is_index=false`) + each index's `chg_pct`/`ltp` (`is_index=true`, name = db symbol e.g. `^NSEI`). Feeds the dashboard **📈 Sector Trend** tab (60s poll, SVG intraday graphs). Anon-read RLS. **Day-scoped:** the streamer wipes the table at 15:35 IST and exits; on startup it also deletes any bars older than today's 9:15 session start. |
...
| `ingestion/fetch_metals.py` | **Metals ingest** — gold/silver futures + DXY + 10Y → `metals_ohlcv`, three resolutions in one run. COPY-to-temp + `ON CONFLICT`; prunes old intraday rows. Run by `metals_ingest.yml`. |
| `ingestion/backfill_intraday.py` | **Intraday backfill** — Yahoo 1h + 5m for the top N by market cap → `ohlcv_hourly` / `ohlcv_5min`. COPY-to-temp then `ON CONFLICT` merge (far faster than row-wise inserts at ~1.7M rows). `--interval 1h|5m|both --days 30 --top 1000 --resume`. Note `--days` maps to Yahoo's period string, which for intraday means ~N *trading* days. |
| `ingestion/scan_trendday.py` | **Trend Day** — scores intraday +6–10% builders off `ohlcv_hourly` → `scanner_trendday`. Called from `fetch_hourly.py` (not `scan_all.py`; needs intraday bars). Calibrated on 16,434 stock-days, top 500, 33 sessions to 2026-08-26; base rate 1.05%. Recalibrate as hourly history grows. |
| `trading/kite_trader.py` | **Automated execution (added 2026-08-30)** — trades the Trend Day scanner via Kite. **DRY RUN by default**; live needs BOTH `--live` AND `KITE_TRADING_ARMED=yes`. `trading/KILL` file forces flat. Risk: Rs 3,000/position, max 3 concurrent, day cap -Rs 10,000 realised, stop = low of day frozen at entry, floored at 1.5% (`MIN_STOP_PCT`) and capped at Rs 2L notional (`MAX_NOTIONAL`) — without those the sizing rule demanded Rs 6.6 crore on one name. `MAX_CUM_PCT=10` stops it chasing past the target band. 50% off at +1%, rest at +3%, square-off 15:10. State in `trading/state_<date>.json`. **Backtested NEGATIVE for the default `ENTRY_STAGES=('CONFIRMED',)`: -Rs 125/trade over 78 trades after costs** — see the module docstring. |
...
| `trendday_scan.yml` | `2,12,22,32 5 * * 1-5` (10:32-11:02 IST) | The day's trend-day setup list, once the 09:15 hourly bar has closed at 10:15 IST. Four slots + a gate on whether today's scan already ran, because GitHub drops cron slots; shares the `hourly-ingest` concurrency group. Runs `fetch_hourly.py`, which ingests bars and calls `scan_trendday`. Added 2026-08-31. |
| `metals_ingest.yml` | `8,38 * * * 0-5` (all day Sun–Fri) | `fetch_metals.py` — feeds the Bullion Desk artifact. Two slots/hour because GitHub drops cron slots; each run is idempotent. |
...
- Tabs (2026-07-07): Breadth (default), **Fresh Breakouts**, **RVOL Daily**, Episodic Pivot, VCP, **VCP Pro**, Manas Scan, Sentiment, Sectors, **Hourly F&O**, **Trend Day**, F&O Sectors, Ticks
...
- **Trend Day tab** = `scanner_trendday`, 5-min poll. `score` is fixed from the 10:15 bar and re-weighted once 11:15 lands — it never reads later bars, so it stays a forecast; `stage` is live state and *is* recomputed from the latest bar. Defaults to score ≥ 20% excluding FADING. Empty outside market hours.
...
- **Sentiment tab** = `aaii_sentiment` (AAII weekly US investor survey). Current bull/neutral/bear cards vs long-run averages (37.5/31.5/31.0), bull−bear spread, and an SVG bull-vs-bear history chart. **Updated weekly by `ingestion/fetch_aaii_web.py` (added 2026-07-31)** — Playwright headless-Chromium scrape of aaii.com/sentimentsurvey (the .xls download in `fetch_aaii.py` is blocked by Imperva bot protection; the web page's 4-week table is parsed instead, site Wednesday dates +1 day → Thursday convention). Scheduled via Windows Task Scheduler task "AAII Sentiment Weekly" (Sat 14:30 local = 10:00 IST, `-StartWhenAvailable`, logs to `ingestion/aaii_weekly.log`).
