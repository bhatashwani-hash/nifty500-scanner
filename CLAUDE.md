...
- **Dashboard:** `dashboard/us.html` (copy at `C:\Users\zashw\nifty500_scanner\dashboard_us.html`) — "Daily US Scan": editorial scroll page (Fraunces serif, dark), hero SPX sparkline w/ 50/200-DMA + weekly trend-vs-range read (Kaufman efficiency), regime-themed via `html[data-regime]` = bull/bear/mixed from SPX vs 50/200-DMA (computed client-side from `us_index_ohlcv`) — green/red/amber background. Chapters: Breadth (SPX candles + 4% bars + %>200DMA), Fresh Breakouts, RVOL, EP (earnings filter), VCP, Sentiment (AAII), Sectors; candlestick modal reads `us_ohlcv`. No Bootstrap — jQuery DataTables core + lightweight-charts only.
