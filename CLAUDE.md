...
- **Dashboard:** `dashboard/us.html` (copy at `C:\Users\zashw\nifty500_scanner\dashboard_us.html`) — "Daily US Scan": editorial scroll page (Fraunces serif, dark), hero SPX sparkline w/ 50/200-DMA + weekly trend-vs-range read (Kaufman efficiency), regime-themed via `html[data-regime]` = super (above 10/20/50/200-DMA, brighter green)/bull/bear/mixed from SPX vs MAs (computed client-side from `us_index_ohlcv`) — green/red/amber background. Chapters: Breadth (SPX candles + 4% bars + %>200DMA), Fresh Breakouts, RVOL, EP (earnings filter), VCP, Sentiment (AAII), Sectors; candlestick modal reads `us_ohlcv`. No Bootstrap — jQuery DataTables core + lightweight-charts only.
- **Seeding:** done 2026-07-07 (3,955 symbols, 1yr history). Re-seed if needed: Actions → "US Daily Ingestion" with `full_backfill=true` (~20 min), then "US Daily Scan".
